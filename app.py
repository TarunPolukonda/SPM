...
Session(app)
mydb=mysql.connector.connect(host='localhost',user='root',password='root',db='spm')
...
def signin():
    if request.method=='POST':
        print(request.form)
        stu_fname=request.form['f_name']
        stu_lname=request.form['l_name']
...
def update(nid):
    if not session.get('email'):
        ...
        if request.method=='POST':
            ...
            mydb.commit()
            cursor.close()
            flash(f'notes updated successfully...')
            return redirect(url_for('panel',nid=nid))
...
def delete(nid):
    ...
    mydb.commit()
    cursor.close()
    flash(f'notes deleted successfuylly...')
    return redirect(url_for('panel',nid=nid))
...
def fileupload():
    if not session.get('email'):
        ...
        if request.method=='POST':
            ...
            mydb.commit()
            cursor.close()
            flash(f'file {file_name} added successfully...')
            return redirect(url_for('panel'))
